...
# Check for the Wix REST API when credentials are configured
if [ -n "${WIX_API_KEY:-}" ] || grep -qE '^WIX_API_KEY=..*' .env 2>/dev/null; then
    ENDPOINTS+=("https://www.wixapis.com/site-properties/v4/properties")
fi

# Published site URL, when configured
if [ -n "${WIX_SITE_URL:-}" ]; then
    ENDPOINTS+=("$WIX_SITE_URL")
fi
