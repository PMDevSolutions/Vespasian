#!/bin/bash
# Figma-to-Wix Comparison Report Generator
# Creates a report comparing the source Figma design to the generated Wix
# build artifacts (tokens.json + BuildPlan + staged global.css).
# Runs on agent completion. Always exits 0.

echo "Generating Figma-to-Wix comparison report..." >&2

# Create reports directory
mkdir -p .claude/reports

REPORT_FILE=".claude/reports/figma-wix-comparison.md"
TIMESTAMP=$(date '+%Y-%m-%d %H:%M:%S')

cat > "$REPORT_FILE" << EOF
# Figma-to-Wix Conversion Report

**Generated:** $TIMESTAMP

## Summary

This report documents the conversion of a Figma design into Wix build
...
if [ -f ".claude/logs/figma-access.log" ]; then
    ERROR_COUNT=$(grep -ci "error\|failed" ".claude/logs/figma-access.log" 2>/dev/null || echo "0")
    echo "- Figma MCP calls logged: $(wc -l < .claude/logs/figma-access.log | tr -d ' ')" >> "$REPORT_FILE"
...
echo "---" >> "$REPORT_FILE"
echo "" >> "$REPORT_FILE"
echo "*Report generated by the figma-wix-converter agent*" >> "$REPORT_FILE"
