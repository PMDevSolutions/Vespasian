...
description: Use this agent when you need to write new tests, run existing tests, analyze failures, and fix them after code changes
...
hooks:
  PreToolUse:
    - matcher: "Bash"
      hooks:
        - type: command
          command: "./scripts/test-writer-fixer/validate-test-command.sh"
  PostToolUse:
    - matcher: "Bash"
      hooks:
        - type: command
          command: "./scripts/test-writer-fixer/save-coverage.sh"
    - matcher: "Write|Edit"
      hooks:
        - type: command
  ...
  Stop:
    - matcher: ".*"
      hooks:
        - type: command
          command: "./scripts/test-writer-fixer/commit-coverage.sh"
---
...
- This project: `node --test` (`*.test.mjs`) for `packages/pipeline` and `packages/wix-driver`, bats for shell scripts, Playwright for editor-flow and published-site checks; all Wix-touching tests run under `VESPASIAN_DRY_RUN=1` — no real network calls in any test
