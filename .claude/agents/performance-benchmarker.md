...
tools: Bash, Read, Write, Grep, MultiEdit, WebFetch, AskUserQuestion, TaskOutput, Edits, Glob, KillShell, Skill, Task, TodoWrite, WebSearch
model: opus
permissionMode: bypassPermissions
hooks:
  SubagentStart:
    - matcher: "performance-benchmarker"
      hooks:
        - type: command
          command: "./scripts/performance-benchmarker/check-tools.sh"
  PostToolUse:
    - matcher: "Bash"
      hooks:
        - type: command
          command: "./scripts/performance-benchmarker/save-benchmarks.sh"
    - matcher: "Write|Edit"
      hooks:
        - type: command
  ...
  Stop:
    - matcher: ".*"
      hooks:
        - type: command
          command: "./scripts/performance-benchmarker/compare-results.sh"
---
...
In this project your most common target is the **published Wix site**: run Lighthouse via lhci (`pnpm lighthouse:run`, configured in lighthouserc.json) against the published URL at the three Studio breakpoints, and profile what the pipeline actually controls — image sizing/optimization before Media upload (`scripts/assets/optimize-images.sh`), `global.css` weight, custom-embed script cost, and Velo code efficiency. Wix owns the serving stack (CDN, SSR); report platform-level costs honestly as context rather than as fixable findings, and route fixes through BuildPlan/`global.css` changes applied by `wix-site-builder`.
